...
type GVMDReportContent struct {
	...
	Task          GVMDReportTask `xml:"task" json:"task"`
	BatchID       string         `json:"batch_id,omitempty"`
}

type GVMDReportTask struct {
	Name string `xml:"name" json:"name"`
}
...
func (s *OpenVASService) GetVersion(ctx context.Context) (string, error) {
	out, err := s.RunGVMCLI(ctx, "<get_version/>")
	if err != nil {
		return "", fmt.Errorf("failed to get OpenVAS version: %w, output: %s", err, string(out))
	}
	return s.extractCleanXML(string(out)), nil
}

func (s *OpenVASService) StartScan(ctx context.Context, target string, batchID string) (map[string]interface{}, error) {
	logger.Info("[OPENVAS_SERVICE] Starting scan for target=%s batch_id=%s", target, batchID)
	namePrefix := "Scan"
	...
	// 1. Create Target
	logger.Info("[OPENVAS_SERVICE] Creating target: %s", targetName)
	...
	if err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to create target: %v", err)
		return nil, fmt.Errorf("failed to create target: %w, output: %s", err, string(out))
	}

	targetID := s.extractIDFromXML(string(out))
	if targetID == "" {
		logger.Error("[OPENVAS_SERVICE] Failed to extract target ID")
		return nil, fmt.Errorf("failed to extract target ID, output: %s", string(out))
	}
	logger.Info("[OPENVAS_SERVICE] Target created with ID: %s", targetID)

	// 2. Create Task
	logger.Info("[OPENVAS_SERVICE] Creating task")
	...
	if err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to create task: %v", err)
		return nil, fmt.Errorf("failed to create task: %w, output: %s", err, string(out))
	}

	taskID := s.extractIDFromXML(string(out))
	if taskID == "" {
		logger.Error("[OPENVAS_SERVICE] Failed to extract task ID")
		return nil, fmt.Errorf("failed to extract task ID, output: %s", string(out))
	}
	logger.Info("[OPENVAS_SERVICE] Task created with ID: %s", taskID)

	// 3. Start Task
	logger.Info("[OPENVAS_SERVICE] Starting task %s", taskID)
	startTaskXML := fmt.Sprintf(`<start_task task_id="%s"/>`, taskID)
	out, err = s.RunGVMCLI(ctx, startTaskXML)
	if err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to start task: %v", err)
		return nil, fmt.Errorf("failed to start task: %w, output: %s", err, string(out))
	}

	logger.Info("[OPENVAS_SERVICE] Scan started successfully, task_id=%s", taskID)
...
func (s *OpenVASService) extractBatchID(name string) string {
...
func (s *OpenVASService) GetTaskStatus(ctx context.Context, taskID string) (*GVMDTask, error) {
	logger.Info("[OPENVAS_SERVICE] Getting task status for task_id=%s", taskID)
	xmlCmd := fmt.Sprintf(`<get_tasks task_id="%s" details="1"/>`, taskID)
	out, err := s.RunGVMCLI(ctx, xmlCmd)
	if err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to get task status: %v", err)
		return nil, err
	}
	...
	if err := xml.Unmarshal([]byte(cleanXML), &resp); err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to parse task XML: %v", err)
	...
	logger.Info("[OPENVAS_SERVICE] Task status retrieved: status=%s progress=%s", resp.Task.Status, resp.Task.Progress)

	return &resp.Task, nil
}

func (s *OpenVASService) GetScanReport(ctx context.Context, reportID string) (*GVMDReportContent, error) {
	logger.Info("[OPENVAS_SERVICE] Getting scan report for report_id=%s", reportID)
	xmlCmd := fmt.Sprintf(`<get_reports report_id="%s" format_id="a994b278-1f62-11e1-96ac-406186ea4fc5" details="1"/>`, reportID)
	out, err := s.RunGVMCLI(ctx, xmlCmd)
	if err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to get scan report: %v", err)
		return nil, err
	}
	...
	if err := xml.Unmarshal([]byte(cleanXML), &resp); err != nil {
		logger.Error("[OPENVAS_SERVICE] Failed to parse report XML: %v", err)
	...
	logger.Info("[OPENVAS_SERVICE] Report retrieved successfully with %d results", len(resp.Report.InnerReport.Results.Result))
