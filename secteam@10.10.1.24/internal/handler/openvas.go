...
func (h *OpenVASHandler) GetVersion(c *fiber.Ctx) error {
	log.Printf("[OPENVAS] GetVersion request")
	ctx, cancel := context.WithTimeout(c.Context(), 30*time.Second)
	defer cancel()

	ver, err := h.service.GetVersion(ctx)
	if err != nil {
		log.Printf("[OPENVAS] Failed to get version: %v", err)
		return response.InternalServerError(c, "Failed to get OpenVAS version", err)
	}
	log.Printf("[OPENVAS] Version retrieved successfully")
...
func (h *OpenVASHandler) StartScan(c *fiber.Ctx) error {
	log.Printf("[OPENVAS] Received scan request")
	var req struct {
	...
	if err := c.BodyParser(&req); err != nil {
		log.Printf("[OPENVAS] Failed to parse request body: %v", err)
		return response.BadRequest(c, "Invalid request payload", err)
	}

	if req.BatchID == "" {
		log.Printf("[OPENVAS] Missing batch_id")
		return response.BadRequest(c, "batch_id is required", nil)
	}

	// Enforce batch ownership
	log.Printf("[OPENVAS] Validating batch ownership for batch_id=%s", req.BatchID)
	if err := h.batchService.ValidateBatchOwnership(c, req.BatchID); err != nil {
		log.Printf("[OPENVAS] Batch ownership validation failed: %v", err)
		return err
	}

	if req.Target == "" {
		log.Printf("[OPENVAS] Missing target")
	...
	log.Printf("[OPENVAS] Starting scan on target=%s", req.Target)
	ctx, cancel := context.WithTimeout(c.Context(), 60*time.Second)
	defer cancel()

	result, err := h.service.StartScan(ctx, req.Target, req.BatchID)
	if err != nil {
		log.Printf("[OPENVAS] Failed to start scan: %v", err)
		return response.InternalServerError(c, "Failed to start OpenVAS scan", err)
	}

	log.Printf("[OPENVAS] Scan started successfully")
...
func (h *OpenVASHandler) GetTaskStatus(c *fiber.Ctx) error {
	...
	if taskID == "" {
		log.Printf("[OPENVAS] GetTaskStatus: Missing task ID")
		return response.BadRequest(c, "Task ID is required", nil)
	}

	log.Printf("[OPENVAS] GetTaskStatus for task_id=%s", taskID)
	ctx, cancel := context.WithTimeout(c.Context(), 30*time.Second)
	defer cancel()

	status, err := h.service.GetTaskStatus(ctx, taskID)
	if err != nil {
		log.Printf("[OPENVAS] Failed to get task status: %v", err)
	...
	log.Printf("[OPENVAS] Task status retrieved: status=%s progress=%s", status.Status, status.Progress)
	return c.JSON(res)
}
...
func (h *OpenVASHandler) GetScanReport(c *fiber.Ctx) error {
	...
	if reportID == "" {
		log.Printf("[OPENVAS] GetScanReport: Missing report ID")
		return response.BadRequest(c, "Report ID is required", nil)
	}

	log.Printf("[OPENVAS] GetScanReport for report_id=%s", reportID)
	ctx, cancel := context.WithTimeout(c.Context(), 120*time.Second)
	defer cancel()

	report, err := h.service.GetScanReport(ctx, reportID)
	if err != nil {
		log.Printf("[OPENVAS] Failed to get report: %v", err)
		return response.InternalServerError(c, "Failed to get report", err)
	}
	log.Printf("[OPENVAS] Report retrieved successfully")
	...
	if h.scanRepo != nil {
		...
		if batchID != "" {
			...
			if dbErr != nil {
				log.Printf("[OPENVAS] Failed to save to database: %v", dbErr)
				return response.InternalServerError(c, "Failed to save scan result", dbErr)
			}
			log.Printf("[OPENVAS] Database insert success")
		}
	}
	
	log.Printf("[OPENVAS] Request completed successfully")
