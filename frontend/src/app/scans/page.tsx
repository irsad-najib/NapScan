...
export default function ScansPage() {
  ...
  const handleStartScan = async () => {
    ...
    try {
      console.log("Starting scan with data:", target);

      for (const tool of selectedTools as ToolKey[]) {
        const result =
          tool === "nmap"
            ? await scannersApi.nmap.scan(target)
            : tool === "zap"
            ? await scannersApi.zap.scan(target)
            : tool === "openvas"
            ? await scannersApi.openvas.scan(target, formData.scanName)
            : tool === "nuclei"
            ? await scannersApi.nuclei.scan(target)
            : tool === "sslyze"
            ? await scannersApi.sslyze.scan(target)
            : await scannersApi.ffuf.scan(target);

        if (!result.ok) {
          const details = result.details ? `\n\n${result.details}` : "";
          throw new Error(
            `${tool.toUpperCase()} failed: ${result.message}${details}`
          );
        }

        console.log(`${tool} scan response:`, result.data);
      }
                      ...
                      {scans.map((scan) => (
                        <tr
                          ...
                          <td className="py-4 px-6 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold ${scan.statusColor}`}>
                              {scan.status}
                            </span>
                          </td>
                          <td className="py-4 px-6">
                            <span
                              className={`text-sm font-bold ${scan.riskColor}`}>
                              {scan.riskLevel}
                            </span>
                          </td>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-500/10 rounded-lg p-4 border border-yellow-200 dark:border-yellow-500/30">
                    <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                      {selectedScanDetails.vulnerabilities}
                    </p>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-4 pb-3 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold ${selectedScanDetails.statusColor}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-3 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                      ...
                      <span
                        className={`text-sm font-bold ${selectedScanDetails.riskColor}`}>
                        {selectedScanDetails.riskLevel}
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      {selectedScanDetails.vulnerabilities} vulnerability
                      item(s) detected during scan execution
                    </p>
                    <div className="mt-4 space-y-2">
                      {[
                        ...Array(
                          Math.min(selectedScanDetails.vulnerabilities, 3)
                        ),
                      ].map((_, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-3 p-3 bg-white dark:bg-slate-700/30 rounded-lg border border-slate-200 dark:border-slate-700">
                          <span className="material-symbols-outlined text-amber-600 dark:text-amber-400 text-lg mt-1">
                            warning
                          </span>
                          <div>
                            <p className="text-sm font-semibold text-slate-900 dark:text-white">
                              Medium Severity Issue #{i + 1}
                            </p>
                      ...
                      {selectedScanDetails.vulnerabilities > 3 && (
                        <p className="text-xs text-slate-600 dark:text-slate-400 text-center py-2">
                          +{selectedScanDetails.vulnerabilities - 3} more
                          vulnerabilities
                        </p>
                      )}
                    </div>
